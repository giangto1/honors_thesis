...
def solve_network_lasso_cvxpy(X, Y, B, Graph, lambda_):
    ...
    loss = 0.5 * cp.sum_squares(Y - cp.sum(cp.multiply(X, B), axis=1)) 
...
if __name__ == '__main__':
    ...
    # pgd_obj = g(X,Y,initB_pgd) + h(initB_pgd, lambda_, Graph)

    # print("initial pgd obj: ", pgd_obj)

    # cvx_obj = g(X,Y,initB_cvx) + h(initB_cvx, lambda_, Graph)
    # print('initial cvx_obj: ', cvx_obj)

    B_cvx, obj_cvx = solve_network_lasso_cvxpy(X, Y, B=initB_cvx, Graph=Graph, lambda_=lambda_)
    # print("Initial B:\n", origB)
    print("\nOptimized B CVX:\n", B_cvx)
    print()
