...
def solveZ(data):
    ...
    # print("lamb:",lamb)
    if(useConvex == 1):
        theta = max(1 - lamb * weight / (rho * LA.norm(a - b) +
                                         0.000001), 0.5)  # So no divide by zero error
        # print("theta:",theta)
        ...
        if(rho * math.pow(d + epsilon, 2) - 8 * c >= 0):
            theta1 = (rho * (d + epsilon) + math.sqrt(math.pow(rho, 2) *
                                                      math.pow(d + epsilon, 2) - 8 * rho * c)) / (4 * rho * d + 0.00000001)
            theta1 = min(max(theta1, 0), 0.5)
            phi = math.log(1 + d * (1 - 2 * theta1) / epsilon)
            objective1 = c * phi + rho * math.pow(d, 2) * (math.pow(theta1, 2))

            theta2 = (rho * (d + epsilon) - math.sqrt(math.pow(rho, 2) *
                                                      math.pow(d + epsilon, 2) - 8 * rho * c)) / (4 * rho * d + 0.00000001)
